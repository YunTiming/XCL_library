...
public class LnChart extends AxesChart {
	...
	protected float getLnXValPosition(double xValue, double maxValue, double minValue) {
		...
		double xScale = MathHelper.getInstance().div(MathHelper.getInstance().sub(xValue, minValue), maxminRange);
...
// 2016-03-15修改  使得SplinChart的X轴能绘制在Y轴的任意点。---------------------------------------------------
	@Override
	...
	protected void drawClipDataAxisGridlines(Canvas canvas) {
		...
		for (int i = 0; i < tickCount + 1; i++) {
			...
			case VERTICAL_CENTER:
				...
				// 从左到右的横向网格线
				drawHorizontalGridLines(canvas, plotArea.getLeft(), plotArea.getRight(), i, tickCount + 1, YSteps,
						currentY);
				...
				// 标签
				currentTickLabel = MathHelper.getInstance().add(dataAxis.getAxisMin(),
						mul(i, (float) dataAxis.getAxisSteps()));
			...
			case HORIZONTAL_CENTER:
				...
				// 绘制竖向网格线
				drawVerticalGridLines(canvas, plotArea.getTop(), plotArea.getBottom(), i, tickCount + 1, XSteps,
						currentX);
	...
	protected void drawClipCategoryAxisGridlines(Canvas canvas) {
		...
		for (int i = 0; i < tickCount; i++) {
			...
			case VERTICAL_CENTER:
				...
				// 从左到右的横向网格线
				drawHorizontalGridLines(canvas, plotArea.getLeft(), plotArea.getRight(), i, tickCount, YSteps,
						currentY);

				if (!categoryAxis.isShowAxisLabels())//如果不显示轴标签
					continue;
				// mLstCateTick.add(new PlotAxisTick(axisX, currentY,
				// dataSet.get(i)));
	...
	protected void renderBezierCurveLine(Canvas canvas, Paint paint, Path bezierPath, List<PointF> lstPoints) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (lstPoints.get(i - 1).y >= axisMinValue && lstPoints.get(i).y >= axisMinValue) {
				...
				if (lstPoints.get(i - 2).y >= axisMinValue) {// 连续3个点为0
					...
					CurveHelper.curve3(lstPoints.get(i - 2), lstPoints.get(i - 1), lstPoints.get(i - 3),
							lstPoints.get(i), BezierControls);
					bezierPath.quadTo(BezierControls[0].x, BezierControls[0].y, lstPoints.get(i - 1).x,
							lstPoints.get(i - 1).y);
				...
				// i-1与i之间的直线
				canvas.drawLine(lstPoints.get(i - 1).x, lstPoints.get(i - 1).y, lstPoints.get(i).x, lstPoints.get(i).y,
						paint);

				continue;
			}

			CurveHelper.curve3(lstPoints.get(i - 2), lstPoints.get(i - 1), lstPoints.get(i - 3), lstPoints.get(i),
					BezierControls);

			// change by chenqiang
			renderBezierCurvePath(canvas, paint, bezierPath, lstPoints.get(i - 2), lstPoints.get(i - 1),
					BezierControls);
		...
		if (count > 3) {
			...
			renderBezierCurvePath(canvas, paint, bezierPath, lstPoints.get(count - 2), lstPoints.get(count - 1),
					BezierControls);
		}
	...
	// 绘制曲线
	private void renderBezierCurvePath(Canvas canvas, Paint paint, Path bezierPath, PointF start, PointF stop,
			PointF[] bezierControls) {
		if (null == bezierPath)
			bezierPath = new Path();
		bezierPath.reset();
	...
	protected void bezierCurvePathAxisMinValue(Path bezierPath, PointF start, PointF stop, PointF[] bezierControls) {
		...
		if (start.y >= axisMinValue && stop.y >= axisMinValue) {
			...
			if (bezierControls[0].y >= axisMinValue && bezierControls[1].y >= axisMinValue) {
				...
				bezierPath.cubicTo(bezierControls[0].x, axisMinValue, bezierControls[1].x, bezierControls[1].y, stop.x,
						stop.y);
			} else if (bezierControls[0].y < axisMinValue && bezierControls[1].y >= axisMinValue) {
				bezierPath.cubicTo(bezierControls[0].x, bezierControls[0].y, bezierControls[1].x, axisMinValue, stop.x,
						stop.y);
			} else {
				bezierPath.cubicTo(bezierControls[0].x, bezierControls[0].y, bezierControls[1].x, bezierControls[1].y,
						stop.x, stop.y);
			}
		}
	}
