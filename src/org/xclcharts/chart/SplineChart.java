...
// System.out.println("setCategories");
	...
	private boolean renderLine(Canvas canvas, SplineData spData, List<PointF> lstPoints) {
		...
		for (int i = 0; i < count; i++) {
			...
			DrawHelper.getInstance().drawLine(spData.getLineStyle(), pointStart.x, pointStart.y, pointStop.x,
					pointStop.y, canvas, spData.getLinePaint());
		}
		return true;
	}
	...
	private boolean renderDotAndLabel(Canvas canvas, SplineData spData, int dataID, List<PointF> lstPoints) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (!pLine.getDotStyle().equals(XEnum.DotStyle.HIDE)) {
				PlotDotRender.getInstance().renderDot(canvas, pDot, dotInfo.mX, dotInfo.mY, pLine.getDotPaint()); // 标识图形

				savePointRecord(dataID, i, dotInfo.mX + mMoveX, dotInfo.mY + mMoveY, dotInfo.mX - radius + mMoveX,
						dotInfo.mY - radius + mMoveY, dotInfo.mX + radius + mMoveX, dotInfo.mY + radius + mMoveY);
				// childID++;
			}
			...
			if (spData.getLabelVisible()) {
				...
				spData.getLabelOptions().drawLabel(canvas, pLine.getDotLabelPaint(),
						getFormatterDotLabel(dotInfo.getLabel()), dotInfo.mX, dotInfo.mY, itemAngle,
						spData.getLineColor());
			}
		}
	...
	protected void drawClipPlot(Canvas canvas) {
		if (renderPlot(canvas) == true) {
			...
			{
				mXAxisCustomLine.renderCategoryAxisCustomlines(canvas, this.getPlotScreenWidth(), this.plotArea,
						mMaxValue, mMinValue);
			}
